...
import { Controller, Delete, Post, Get, ClassMiddleware, Put } from "@overnightjs/core";
...
export class BlogController {
    ...
    async updateBlog(req: Request, res: Response, next: NextFunction) {
        try {
            const blogDto: BlogIdDto = plainToClass(BlogIdDto, req.body);
            const blogID: BlogIdDto = plainToClass(BlogIdDto, req.params)
            const blog: IBlog = await this.blogService.updateOne(blogID,blogDto);
            return res.status(201).json({
                statusCode: 201,
                message: "updated",
                data: { blog , blogDto}
